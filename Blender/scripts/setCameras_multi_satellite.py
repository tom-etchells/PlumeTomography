...
def run(numSats, crossTrack, focalLength):
    # %% Camera Positions
    print("Num Sats = {}".format(numSats))
    camPos = []
    if numSats > 1:
        for satNum in range(numSats):
            camPos.extend(satPos(crossTrack[satNum]))
    else:
        camPos = satPos(crossTrack)

    numCams = len(camPos)
    print("Num Cams = {}".format(numCams))
